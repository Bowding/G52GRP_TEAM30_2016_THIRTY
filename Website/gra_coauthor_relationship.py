...
cur = conn.cursor()
cur.execute("SELECT * FROM `connections` WHERE `sourceScholar` = '%s';" % (search))
...
if result > 0:
    ...
    for element in targets:
        query = "SELECT * FROM `connections` WHERE `sourceScholar` = '%s' AND (" % (element)
        idx = 0
        for target in targets:
            if idx == 0:
    ...
    for element in targets:
        query = "SELECT * FROM `institutions` WHERE `scholarName` = '%s';" % (element)
        cur.execute(query)
